"""
Gesture-Based Password Login System (MediaPipe Tasks API)
...
""" Download the required MediaPipe Hand Landmarker model if it doesn't exist locally """
...
""" Define hand skeleton connections for manual drawing """
HAND_CONNECTIONS = [
...
def draw_landmarks_manual(img, landmarks):
    """ Draws the hand landmarks and connecting lines manually using OpenCV """
    h, w, _ = img.shape
    points = []
    for lm in landmarks:
...
def main():
    ...
    print(f"System Armed. Please enter the {len(PASSWORD_SEQUENCE)}-gesture password.")
    ...
    while True:
        ...
        """ Convert OpenCV image to MediaPipe Image format and process """
        ...
        if result.hand_landmarks:
            for hand_landmarks, handedness in zip(result.hand_landmarks, result.handedness):
                ...
                """ Calculate thumb state using X-coordinates (Thumb Tip = 4, Thumb IP = 3) """
                ...
                """ Calculate other 4 fingers using Y-coordinates (tip vs PIP joint 2 nodes down) """
        ...
        if unlocked:
            ...
            cv2.putText(img, "ACCESS GRANTED", (100, 250), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 4)
        else:
            cv2.putText(img, f"Fingers: {fingers_up_count}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            cv2.putText(img, f"Step: {current_step}/{len(PASSWORD_SEQUENCE)}", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
